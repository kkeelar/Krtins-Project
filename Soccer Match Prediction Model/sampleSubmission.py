...
def sse_score(df):
    outcomes = df["goals2H"].to_numpy()
    preds = df["predGoals2H"].to_numpy()
    return np.sum((preds - outcomes) ** 2)
...
if __name__ == "__main__":
    df = pd.read_csv("soccermatches.csv").dropna(subset=[
        "HG_LY","HGA_LY","HM_LY","AG_LY","AGA_LY","AM_LY","goals2H"
    ])
    
    # Run the SAMPLE baseline
    df = calc_my_goals_second_half(df)
    print("Sample Function 3 SSE:", sse_score(df))
    print("Sample Function 3 MSE:", mse_score(df))
