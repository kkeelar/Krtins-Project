...
def run_strategy_comparison(
    data: Dict[str, pd.DataFrame], strategy_configs: List[Dict], risk_free_rate: float = 0.0
) -> pd.DataFrame:
    rows = []
    for config in strategy_configs:
        ...
        for params in config["params"]:
            ...
            sharpe = sharpe_ratio(bt["daily_return"], risk_free_rate=risk_free_rate)
            latency_avg = bt.attrs.get("latency_ns_avg")
            latency_p95 = bt.attrs.get("latency_ns_p95")
            rejections = bt.attrs.get("rejections")
            rows.append(
                {
                    "strategy_name": name,
                    "parameters": params,
                    ...
                    "volatility": vol,
                    "sharpe": sharpe,
                    "latency_ns_avg": latency_avg,
                    "latency_ns_p95": latency_p95,
...
if __name__ == "__main__":
    ...
    summary = run_strategy_comparison(prices, configs, risk_free_rate=0.02)
    ...
    # persist best row to summary_runs.csv
    best = summary.iloc[0]
    summary_runs_path = RESULTS_DIR / "summary_runs.csv"
    best_row = {
        "run_name": f"sweep_best_{best['strategy_name']}",
        "description": f"Best sweep result ({best['strategy_name']}, {format_parameters(best['parameters'])})",
        ...
        "hit_rate": None,
        "turnover": None,
        "latency_ns_avg": best.get("latency_ns_avg"),
        "latency_ns_p95": best.get("latency_ns_p95"),
        "rejections": best.get("rejections"),
    }
    header = list(best_row.keys())
    exists = summary_runs_path.exists()
    with summary_runs_path.open("a", newline="") as f:
