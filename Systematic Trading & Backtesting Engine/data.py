...
def load_data(
    tickers: List[str], start: str, end: str, max_ffill_days: int = 5
    ...
    # Use intersection of calendars to avoid stale cross-asset ffill #
    common_dates = set(raw[tickers[0]].index)
    for df in raw.values():
        common_dates &= set(df.index)
    common_index = pd.DatetimeIndex(sorted(common_dates), name="Date")

    aligned: Dict[str, pd.DataFrame] = {}
    for ticker, df in raw.items():
        # Reindex to common dates, limited forward-fill to avoid long gaps #
        reindexed = df.reindex(common_index).ffill(limit=max_ffill_days).dropna()
